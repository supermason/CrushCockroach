package com.mason.crushcockroach.objPool
{
	...
	public class ObjPool {
		...
		/** Objects in the pool. */
		private var list:Vector.<IPoolable> = new Vector.<IPoolable>();
		...
		*/
		...
		public function ObjPool(create:Function, clean:Function = null, minSize:int = 50, maxSize:int = 200)
		{
			this.create = create;
			this.clean = clean;
		 ...
		 *
		 */
		public function checkOut():IPoolable
		{
			if(length == 0) {
				if(size < maxSize) {
					size++;
		 ...
		 *
		 */
		public function checkIn(item:IPoolable):void
		{
			if(clean != null) clean(item);
			list[length++] = item;
			trace("in checkIn: ", length, "\n||||||||||||||||||||||||||||||||||||||"); 
		}
